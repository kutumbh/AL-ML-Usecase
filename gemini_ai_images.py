...
questions = [
    # "Provide a title of virat kholi with heading as 'Title:' and Introduction of 7 lines with heading as 'Introduction:' and virat kholi images with heading as 'Images:' ",
    ...
    "Provide following details of Virat Kohli - awards with heading 'Awards:' and Felicitations with heading 'Felicitations:' and only jpg Images with urls displayed at last having heading 'Images:' ",
...
def download_images(response):
    ...
    download_dir ='D:/Gemini ai/Images/Introduction'
    image_urls = re.findall(r'\[(.*?)\]\((.*?)\)', response)
    ...
    for i, url in enumerate(image_urls, start=1):
        try:
            ...
            # Encode the URL
            encoded_url = urllib.parse.quote(url_string, safe=':/')
            response = requests.get(encoded_url)
            time.sleep(60)  # Add a delay between requests to avoid being rate-limited
...
def get_values(response,sports_json):
    ...
    if 'Initiation' in response:
        ...
        for line in lines:
            ...
            elif "Domestic Debut:" in line or "International Debut:" in line:
                capture_initiation = False
        ...
        for line in lines:
            ...
            elif  "International Debut:" in line:
                capture_domestic_debut = False
    ...
    #achievments
    if 'Key Achievements:' in response:
        #key achievements
        time.sleep(4)
    ...
    if "Anecdotes:" in response:
        ...
        for line in lines:
            time.sleep(2)
            if line.__contains__("Anecdotes") :
        ...
        for line in lines:
            time.sleep(2)
            if line.__contains__("Controversies") :
    ...
    #personality attributes
    if 'Personality Attributes:' in response:
        time.sleep(4)
        personality_details=''
        ...
        for line in lines:
            time.sleep(2)
            if line.__contains__("Personality Attributes") :
    ...
    if 'Society:' in response:
        ...
        for line in lines:
            time.sleep(2)
            if line.__contains__("Philanthropy") :
...
# with open('Json_files/famous_personality_virat_kholi_images.json', 'w') as json_file:
#     json_file.write(json_output)

# print("json has been saved to 'famous_personality_virat_kholi_images.json'")
